...
def test_fft_and_back_almost_same(
        V0: float, a: float, b: float, n_dots: int,
        ft: FourierTransformer, square_well_potential):
    assume(V0 < 0)
    assume(a < b)
    kwargs = {'V0': V0, 'a': a, 'b': b}
    ft. \
        add_grid(b/2, n_dots). \
        add_potential(square_well_potential, **kwargs)
    expected = square_well_potential(ft.x_grid, **kwargs)
    actual = ft.convert_back(ft.freq_coeffs)
    np.testing.assert_allclose(actual, expected, atol=abs(V0*0.05))
